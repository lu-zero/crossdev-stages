...
pub fn run(boards_root: &Utf8Path, cmd: BoardCmd) -> Result<()> {
    match cmd {
        ...
        BoardCmd::Info { board: board_name } => {
            ...
            if let Some(t) = &board_cfg.opensbi_tag {
                println!("OpenSBI tag:    {t}");
            }
            if let Some(p) = &board_cfg.opensbi_platform {
                println!("OpenSBI plat:   {p}");
            }
            if let Some(f) = &board_cfg.opensbi_fw_type {
                println!("OpenSBI fw:     {f}");
            }
            if let Some(f) = &board_cfg.opensbi_make_flags {
                println!("OpenSBI flags:  {f}");
            }
            if let Some(r) = &board_cfg.u_boot_repo {
                println!("U-Boot repo:    {r}");
            }
            if let Some(t) = &board_cfg.u_boot_tag {
                println!("U-Boot tag:     {t}");
            }
            if let Some(d) = &board_cfg.u_boot_defconfig {
                println!("U-Boot deconf:  {d}");
            }
