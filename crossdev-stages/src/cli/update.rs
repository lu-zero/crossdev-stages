...
fn check_lock(ws: &Workspace, lock_path: &Utf8Path) -> Result<()> {
    let body =
        std::fs::read_to_string(lock_path).map_err(|e| crate::error::Error::CommandFailed {
            code: 1,
            reason: format!("read {lock_path}: {e}"),
        })?;
    let lock: LockFile = toml::from_str(&body).map_err(|e| crate::error::Error::CommandFailed {
        code: 1,
...
fn check_source(
    ...
    src: &LockSource,
    cache: &Utf8Path,
) -> std::result::Result<SourceState, String> {
    if !cache.exists() {
...
fn locate_all_locks(ws: &Workspace) -> Vec<Utf8PathBuf> {
    let Ok(builds) = ws.list_builds() else {
        return vec![];
    };
    let mut seen = std::collections::BTreeSet::<String>::new();
    let mut out = Vec::new();
    for dir in builds {
        let Ok(board) = std::fs::read_to_string(dir.join(".board")).map(|s| s.trim().to_string())
        else {
            continue;
        };
...
fn lock_has_git_sources(lock_path: &Utf8Path) -> bool {
    ...
    let Ok(parsed) = toml::from_str::<LockFile>(&body) else {
        return false;
    };
    parsed
        .sources
