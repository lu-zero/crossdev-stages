...
impl Stage3Fetcher {
    ...
    fn fetch_stage3_list(&self) -> Result<Vec<Stage3Info>, Stage3Error> {
        ...
        if !output.status.success() {
            ...
            return Err(Stage3Error::FetchError(
                format!("Failed to fetch stage3 list: {}", stderr)
            ));
        }
    ...
    fn find_latest_stage3(&self, images: &[Stage3Info]) -> Result<Stage3Info, Stage3Error> {
        images.iter()
            ...
            .cloned()
            .ok_or_else(|| Stage3Error::ParseError(
                "No stage3 images available".to_string()
            ))
    }
    ...
    fn download_stage3(&self, stage3: &Stage3Info) -> Result<(), Stage3Error> {
        ...
        let output = Command::new("curl")
            ...
            .arg("-o").arg(&cache_path)  // Output to cache file
        ...
        if !output.status.success() {
            ...
            return Err(Stage3Error::DownloadError(
                format!("Failed to download {}: {}", stage3.name, stderr)
            ));
        }
    ...
    fn verify_stage3(&self, stage3: &Stage3Info) -> Result<(), Stage3Error> {
        ...
        if !cache_path.exists() {
            return Err(Stage3Error::VerifyError(
                format!("Stage3 image not found: {}", cache_path.display())
            ));
        }
        ...
        if metadata.len() != stage3.size {
            return Err(Stage3Error::VerifyError(
                format!(
                    ...
                    stage3.name, stage3.size, metadata.len()
                )
            ));
        }
    ...
    pub fn extract_stage3(&self, stage3: &Stage3Info, target_dir: impl AsRef<Path>) -> Result<(), Stage3Error> {
        let cache_path = self.cache_dir.join(&stage3.name);
        let target_dir = target_dir.as_ref();
        
        // Check if cache file exists
        if !cache_path.exists() {
            return Err(Stage3Error::ExtractError(
                format!("Stage3 image not found in cache: {}", cache_path.display())
            ));
        }
        ...
        let output = Command::new("tar")
            .arg("--exclude").arg("dev/*")
            .arg("-xJpf").arg(&cache_path)  // Extract with xz decompression
            .arg("-C").arg(target_dir)      // Change to target directory
            .output()
            .map_err(|e| Stage3Error::IoError(e))?;
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(Stage3Error::ExtractError(
                format!("Failed to extract {}: {}", stage3.name, stderr)
            ));
        }
        
        info!("Stage3 image extracted successfully to: {}", target_dir.display());
