...
# AV1, SCMI cpufreq, PMIC).  Four things it does not enable: the NPU
# (rocket, drivers/accel), the RK3588 video decoder (vdpu381), the IOMMU
# the AV1 decoder sits behind, and the JPEG decoder the patches under
# patches/linux/ add.  av1d in rk3588-base.dtsi points at av1d_mmu,
...
#
# VIDEO_ROCKCHIP_JPEGD is out-of-tree here, so it exists only because
# 0004..0007 applied.  Asking for it is what makes a silently dropped patch
# a build failure instead of an image missing the decoder.  It can only be a
# module: MEDIA_SUPPORT is =m in defconfig.  Its jpegd_mmu is a plain
# rockchip,rk3568-iommu, already covered by CONFIG_ROCKCHIP_IOMMU=y.
cd /build/linux
...
want CONFIG_VIDEO_ROCKCHIP_JPEGD m
