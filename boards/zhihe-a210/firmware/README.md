...
- **bootzero2.bin**: post-assemble uses it to build
  `emmc_boot-loader.img` (bootzero2 + SPL + FIT for eMMC boot0).
  Without it that image is not generated and flash.sh skips the
  `mmc0boot0` stage with a warning — fine if the eMMC boot0 loader is
  already in place from a vendor image, required for a bare board.
